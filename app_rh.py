...
def page_materiais():
    ...
    with tab_cad:
        ...
        tipo_cad = st.selectbox("O que deseja cadastrar?", ["Material", "EPI", "Cliente"], key="cad_tipo")

        if tipo_cad == "Cliente":
            st.caption("Cadastre um novo cliente. Depois você poderá adicionar as lojas, os materiais e os EPIs dele.")
            nome_cli = st.text_input("Nome do novo cliente *", key="cad_novo_cliente")
            primeira_loja = st.text_input("Primeira loja (opcional)", key="cad_novo_cliente_loja")
            if st.button("💾 Salvar Cliente", type="primary", key="cad_salvar_cliente"):
                nc = nome_cli.strip()
                if not nc:
                    ...
                    CLIENTES.append(nc)
                    LOJAS_POR_CLIENTE.setdefault(nc, [])
                    MATERIAIS_POR_CLIENTE.setdefault(nc, [])
                    EPIS_POR_CLIENTE.setdefault(nc, [])
                    _registrar_catalogo("Cliente", nc, "")
                    if primeira_loja.strip():
                        LOJAS_POR_CLIENTE[nc].append(primeira_loja.strip())
                        _registrar_catalogo("Loja", nc, primeira_loja.strip())
                    st.success(f"Cliente '{nc}' cadastrado com sucesso!")
            # Fragmento reexecuta automaticamente
        else:
            ...
            if st.button("💾 Salvar Item", type="primary", key="cad_salvar"):
                if not nome_cad.strip():
                    ...
                    if tipo_cad == "Material":
                        if nome_cad.strip() not in MATERIAIS_POR_CLIENTE.get(cliente_cad, []):
                            ...
                            _registrar_catalogo("Material", cliente_cad, nome_cad.strip())
                ...
                # Fragmento reexecuta automaticamente
                        ...
                        if nome_cad.strip() not in EPIS_POR_CLIENTE.get(cliente_cad, []):
                            ...
                            _registrar_catalogo("EPI", cliente_cad, nome_cad.strip())
...
class ErroLeituraGS(Exception):
    """Falha REAL ao ler uma aba do Google Sheets (conexao/permissao/limite).

    IMPORTANTE: e diferente de 'aba vazia'. Uma aba vazia devolve DataFrame
    vazio normalmente; uma falha de leitura LEVANTA esta excecao para que o
    salvamento NUNCA grave 'vazio' por cima de dados bons.
    """
    pass

def _eh_erro_quota(e):
    """Detecta o erro 429 do Google (limite de requisicoes por minuto)."""
    msg = str(e)
...
def _gs_retry(func, *args, tentativas=4, espera_base=1.0, **kwargs):
    ...
    for i in range(tentativas):
        try:
            ...
            ultima_exc = e
            if _eh_erro_quota(e) and i < tentativas - 1:
                try:
...
def _gs_listar_abas(spreadsheet):
    """Lista as abas da planilha com re-tentativa em caso de 429."""
    return _gs_retry(spreadsheet.worksheets)

def _gsheet_to_df(worksheet):
    """Converte uma worksheet do gspread para DataFrame.

    CORRECAO CRITICA DE SEGURANCA DE DADOS: antes, QUALQUER erro de leitura
    (queda de internet, limite da API, permissao) era silenciosamente tratado
    como 'planilha vazia'. Combinado com o salvamento (que apaga e regrava a
    aba), uma unica falha temporaria de leitura seguida de um salvamento
    APAGAVA todos os dados da nuvem. Agora um erro de leitura LEVANTA excecao
    e so devolvemos vazio quando a aba esta REALMENTE vazia.
    """
    try:
        records = _gs_retry(worksheet.get_all_records)
    except Exception as e:
        # NUNCA tratar falha de leitura como 'vazio'. Propaga o erro.
        raise ErroLeituraGS(str(e))
    if not records:
        return pd.DataFrame()
...
def _df_to_gsheet(df, worksheet):
    ...
    if df is None:
        return
    if df.empty:
        # BLINDAGEM CONTRA PERDA DE DADOS: NUNCA apagar uma aba que hoje tem
        ...
        # 1a linha e o cabecalho; se houver linhas de dados alem dele, ABORTA.
        if len(linhas_com_dados) > 1:
            raise ErroLeituraGS(
                f"Protecao ativada: recusei apagar a aba '{worksheet.title}' "
...
def _carregar_dados_gs():
    ...
    abas = {ws.title: ws for ws in _gs_listar_abas(spreadsheet)}
    dados = {}
...
# ====================== CATALOGO (CLIENTES/LOJAS/MATERIAIS/EPIS) ======================
# O catalogo base (CLIENTES, LOJAS_POR_CLIENTE, etc.) fica fixo no codigo.
# As INCLUSOES feitas pelo usuario (novo cliente, nova loja, novo material/EPI)
# sao guardadas na aba "Catalogo" do Google Sheets (e num espelho local),
# para NAO se perderem quando o app reinicia.
COLS_CATALOGO = ["Tipo", "Cliente", "Item"]


def _catalogo_arquivo_local():
    return os.path.join(DATA_DIR, "catalogo_extra.json")


def _aplicar_linha_catalogo(tipo, cliente, item):
    """Aplica uma inclusao do catalogo nas estruturas em memoria."""
    ...
    if tipo == "cliente":
        ...
        LOJAS_POR_CLIENTE.setdefault(cliente, [])
        MATERIAIS_POR_CLIENTE.setdefault(cliente, [])
        EPIS_POR_CLIENTE.setdefault(cliente, [])
    elif tipo == "loja" and item:
        LOJAS_POR_CLIENTE.setdefault(cliente, [])
        if item not in LOJAS_POR_CLIENTE[cliente]:
            LOJAS_POR_CLIENTE[cliente].append(item)
    elif tipo == "material" and item:
        MATERIAIS_POR_CLIENTE.setdefault(cliente, [])
        if item not in MATERIAIS_POR_CLIENTE[cliente]:
            MATERIAIS_POR_CLIENTE[cliente].append(item)
    elif tipo == "epi" and item:
        EPIS_POR_CLIENTE.setdefault(cliente, [])
        if item not in EPIS_POR_CLIENTE[cliente]:
            EPIS_POR_CLIENTE[cliente].append(item)


def _carregar_catalogo_extra():
    """Le as inclusoes do usuario (Google Sheets ou espelho local) e aplica em memoria."""
    linhas = []
    if GS_ENABLED and (GS_ID_COMPRAS or GS_ID_FUNCIONARIOS):
        try:
            spreadsheet = _abrir_planilha_gs(GS_ID_COMPRAS, GS_ID_FUNCIONARIOS)
            ws = _aba_gs(spreadsheet, "Catalogo", COLS_CATALOGO)
            registros = _gs_retry(ws.get_all_records)
            linhas = [dict(r) for r in registros]
        except Exception:
            linhas = []
    if not linhas and not GS_ONLY:
        try:
            cam = _catalogo_arquivo_local()
    ...
    for r in linhas:
        try:
            _aplicar_linha_catalogo(r.get("Tipo", ""), r.get("Cliente", ""), r.get("Item", ""))
        except Exception:
            continue


def _registrar_catalogo(tipo, cliente, item=""):
    """Grava uma inclusao do catalogo no Google Sheets e no espelho local."""
    tipo = (tipo or "").strip()
    cliente = (cliente or "").strip()
    item = (item or "").strip()
    if not cliente:
    ...
    if GS_ENABLED and (GS_ID_COMPRAS or GS_ID_FUNCIONARIOS):
        try:
            spreadsheet = _abrir_planilha_gs(GS_ID_COMPRAS, GS_ID_FUNCIONARIOS)
            ws = _aba_gs(spreadsheet, "Catalogo", COLS_CATALOGO)
            _gs_retry(ws.append_row, [tipo, cliente, item], value_input_option="USER_ENTERED")
        except Exception as e:
            st.warning(f"⚠️ Salvo na tela, mas falhou o envio para o Google Sheets: {e}")
    if not GS_ONLY:
        try:
            cam = _catalogo_arquivo_local()
            linhas = []
            ...
            if not isinstance(linhas, list):
                linhas = []
            linhas.append({"Tipo": tipo, "Cliente": cliente, "Item": item})
            _salvar_json_seguro(cam, linhas)
        except Exception:
            pass
    try:
        _registrar_log("CADASTROU CATALOGO", f"{tipo} | {cliente} | {item}")
    except Exception:
        pass
    return True
...
if GS_ENABLED:
    try:
        ...
        spreadsheet = _abrir_planilha_gs(GS_ID_FUNCIONARIOS, GS_ID_DIARIAS)
        abas_existentes = {ws.title for ws in _gs_listar_abas(spreadsheet)}
...
def carregar_dados():
    ...
    PROTECAO CONTRA LIMITE DE LEITURA (429): se a nuvem devolver uma base
    vazia por causa de erro/limite temporario, e ja tivermos uma copia boa
    (com registros) guardada nesta sessao, servimos a copia boa em vez de
    mostrar 0 registros. Isso tambem evita que uma tela vazia acabe
    sobrescrevendo os dados na nuvem.
    """
    ...
    # Verifica se a leitura atual trouxe conteudo de verdade
    def _tem_conteudo(d):
        try:
            base = d.get("Base_Dados")
            return isinstance(base, pd.DataFrame) and not base.empty
        except Exception:
            return False

    if _tem_conteudo(dados):
        # Leitura boa: atualiza a copia de seguranca da sessao
        st.session_state["_dados_cache_bom"] = dados
    else:
        # Leitura vazia: se ha uma copia boa guardada, serve ela
        _bom = st.session_state.get("_dados_cache_bom")
        if _bom is not None and _tem_conteudo(_bom):
            return _bom
...
def _invalidar_cache_dados():
    ...
    CORRECAO 429: antes chamava st.cache_data.clear(), que apagava o cache
    de TODOS os modulos (funcionarios, diarias, compras...). Resultado: apos
    qualquer exclusao/gravacao, a proxima tela relia TUDO do Google Sheets de
    uma vez, estourando o limite de leituras por minuto. Agora limpamos apenas
    o cache da base de funcionarios.
    """
    st.session_state.pop("_dados_cache", None)
    st.session_state.pop("_dados_cache_id", None)
    try:
        _carregar_dados_raw.clear()
    except Exception:
        pass
@st.cache_data(ttl=300, show_spinner=False)
...
def _invalidar_cache_diarias():
    ...
    CORRECAO 429: limpa apenas o cache das diarias, sem apagar o cache dos
    demais modulos (evita reler tudo do Google Sheets de uma so vez).
    """
    st.session_state.pop("_diarias_cache", None)
    st.session_state.pop("_diarias_cache_id", None)
    try:
        _carregar_diarias_raw.clear()
    except Exception:
        pass
def salvar_dados(dados):
    ...
    if GS_ONLY:
        try:
            ...
            # Atualiza a copia boa da sessao com os dados recem-salvos.
            # Assim, se a releitura seguinte bater no limite do Google (429),
            # a tela ja mostra o resultado correto (com a exclusao aplicada)
            # em vez de dados antigos ou vazios.
            try:
                st.session_state["_dados_cache_bom"] = dados
            except Exception:
                pass
        ...
        for _, f in dados["Base_Dados"].iterrows():
            ...
            try:
                ...
                if meses >= 12:
                    ...
                    pos_concessivo = -1
                    faltam = -1
                # ALERTA DE 4 MESES (regra atualizada):
                # Mostra quem está a 4 meses ou menos de vencer QUALQUER prazo
                # de férias:
                #   - 1as férias: quem está prestes a completar 24 meses
                #     (ex.: 20m->faltam 4, 21m->faltam 3, ... 23m->faltam 1)
                #   - Próximas férias: quem já tirou e está se aproximando do
                #     vencimento do ciclo seguinte (mesma contagem, a cada 12m)
                #   - E também quem JÁ venceu o prazo (faltam 0 = férias em dobro)
                # faltam já é calculado acima: 12,11,...,1 até o vencimento e 0
                # quando vence. Basta exibir quando faltam de 0 a 4 meses.
                if meses >= 12 and 0 <= faltam <= 4:
...
with aba10:
    ...
    def traduzir_texto(texto, origem="auto", destino="pt"):
        """Traduz texto. Tenta varios metodos em ordem e devolve (resultado, erro).

        Retorna uma tupla: (texto_traduzido | None, mensagem_de_erro | None).
        Assim quem chama pode mostrar a causa real da falha.
        """
        ...
        # --- Metodo 1: API gratuita translate_a/single (endpoint gtx) ---
        try:
            ...
            if dados and isinstance(dados, list) and dados[0]:
                partes = [p[0] for p in dados[0] if isinstance(p, list) and len(p) > 0 and p[0]]
                traduzido = "".join(partes)
                if traduzido.strip():
                    return traduzido, None
        except Exception as e:
            erros.append(f"metodo 1 (googleapis): {e}")

        # --- Metodo 2: endpoint alternativo clients5 ---
        try:
            ...
            dados2 = resp2.json()
            # Formato: [["traducao","idioma_origem"]] ou lista de strings
            if isinstance(dados2, list) and dados2:
                if isinstance(dados2[0], list) and dados2[0]:
                    return str(dados2[0][0]), None
                if isinstance(dados2[0], str):
                    return "".join([x for x in dados2 if isinstance(x, str)]), None
        except Exception as e:
            erros.append(f"metodo 2 (clients5): {e}")

        # --- Metodo 3: biblioteca deep_translator (se instalada) ---
        try:
            from deep_translator import GoogleTranslator
            src = "auto" if origem in ("auto", "") else origem
            traduzido = GoogleTranslator(source=src, target=destino).translate(texto)
            if traduzido and str(traduzido).strip():
                return str(traduzido), None
        except ImportError:
            erros.append("metodo 3 (deep_translator): biblioteca nao instalada")
        except Exception as e:
            erros.append(f"metodo 3 (deep_translator): {e}")

        return None, " | ".join(erros) if erros else "Falha desconhecida."
            ...
            if st.button("🔄 TRADUZIR", type="primary", use_container_width=True):
                if not texto_origem.strip():
                    ...
                    with st.spinner("Traduzindo..."):
                        ...
                        resultado, erro_trad = traduzir_texto(texto_origem, origem=cod_origem, destino=cod_destino)
                    if resultado:
                        ...
                        st.error("❌ Não foi possível traduzir. O serviço de tradução pode estar indisponível ou sem conexão com a internet.")
                        if erro_trad:
            ...
            if gerar_audio:
                if not texto_tts.strip():
                    ...
                    try:
                        ...
                        st.error("❌ A biblioteca `gTTS` não está instalada.")
                        st.info("💡 Para ativar a geração de áudio, adicione `gtts` ao arquivo requirements.txt "
                                "e reinicie o app. Localmente, execute: `pip install gtts`")
                        st.stop()
                    ...
                    if traduzir_antes:
                        with st.spinner("Traduzindo texto..."):
                            ...
                            texto_traduzido, _erro_tts = traduzir_texto(texto_tts, origem=cod_origem, destino=cod_tts)
                        if texto_traduzido:
        ...
        with sub_aba_stt:
                    ...
                    with st.spinner("Processando áudio..."):
                        ...
                        try:
                            ...
                            if texto_transcrito.strip():
                                texto_traduzido, _erro_stt = traduzir_texto(texto_transcrito, origem=cod_stt, destino=cod_trad)
            ...
            if traduzir_novamente:
                if "stt_texto_transcrito_atual" not in st.session_state or not st.session_state["stt_texto_transcrito_atual"]:
                    ...
                    with st.spinner("Traduzindo..."):
                        texto_traduzido, _erro_stt2 = traduzir_texto(texto_transcrito, origem=cod_stt, destino=cod_trad)
                        if texto_traduzido is None:
                            texto_traduzido = ""
                    st.session_state["stt_historico"].append({
                        "id": len(st.session_state["stt_historico"]),
                            ...
                            if st.button("🔊 Ouvir Tradução", use_container_width=True, key=f"stt_ouvir_{idx}"):
                                ...
                                if not texto_ouvir.strip():
                                    ...
                                    try:
                                        ...
                                        st.error("❌ A biblioteca `gTTS` não está instalada.")
                                        st.info("💡 Adicione `gtts` ao requirements.txt e reinicie o app. "
                                                "Localmente: `pip install gtts`")
                                        st.stop()
                                    with st.spinner("Gerando áudio..."):
