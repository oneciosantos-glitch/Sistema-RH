...
# ===== Catch import errors and show real traceback =====
_import_errors = []
...
try:
    ...
    _import_errors.append(f"calculos.py: {_tb.format_exc()}")
...
try:
    ...
    _import_errors.append(f"dashboard.py: {_tb.format_exc()}")
    dash = None  # type: ignore

try:
    import exportador
except Exception as _e:
    _import_errors.append(f"exportador.py: {_tb.format_exc()}")
    exportador = None  # type: ignore

try:
    import estilo as _estilo_real
    estilo = _estilo_real
except Exception as _e:
    _import_errors.append(f"estilo.py: {_tb.format_exc()}")
    _estilo_real = None

    # Fallback: no-op stubs so the app never crashes on estilo.*
    class _EstiloStub:
        CORES_GRAFICO = ['#636EFA', '#EF553B', '#00CC96', '#AB63FA', '#FFA15A',
                         '#19D3F3', '#FF6692', '#B6E880', '#FF97FF', '#FECB52']
        @staticmethod
        def aplicar(*a, **kw): pass
        @staticmethod
        def cabecalho(*a, **kw): pass
        @staticmethod
        def cartao(*a, **kw): pass
        @staticmethod
        def cartao_evento(*a, **kw): pass
        @staticmethod
        def cartao_prazo_experiencia(*a, **kw): pass
        @staticmethod
        def estilo_tabela(*a, **kw): return a[0] if a else None
        @staticmethod
        def estilo_tabela_saas(*a, **kw): return a[0] if a else None
        @staticmethod
        def marca(*a, **kw): pass
        @staticmethod
        def secao_todos_prazos(*a, **kw): pass
        @staticmethod
        def tag(*a, **kw): pass
        @staticmethod
        def alerta_ferias_4_meses(*a, **kw): pass
        @staticmethod
        def alerta_ferias_gozo_proximo(*a, **kw): pass
...
try:
    ...
    _import_errors.append(f"database.py: {_tb.format_exc()}")
    Banco = None  # type: ignore
...
d_turnover_periodo = getattr(dash, "turnover_periodo", _fb_turnover_periodo) if dash else _fb_turnover_periodo
d_resumo_eventos = getattr(dash, "resumo_eventos", _fb_resumo_eventos) if dash else _fb_resumo_eventos
d_ativos = getattr(dash, "ativos", _fb_ativos) if dash else _fb_ativos
d_eventos_experiencia = getattr(dash, "eventos_experiencia", _fb_eventos_experiencia) if dash else _fb_eventos_experiencia
d_eventos_ferias = getattr(dash, "eventos_ferias", _fb_eventos_ferias) if dash else _fb_eventos_ferias
d_movimentacao_mensal = getattr(dash, "movimentacao_mensal", _fb_movimentacao_mensal) if dash else _fb_movimentacao_mensal
d_turnover_por_loja = getattr(dash, "turnover_por_loja", _fb_turnover_por_loja) if dash else _fb_turnover_por_loja
...
def get_banco():
    if Banco is None:
        st.error("❌ Módulo database.py não encontrado. Verifique o repositório.")
        st.stop()
...
def pagina_exportar(banco):
    ...
    if registros:
        if exportador is None:
            st.warning("⚠️ Módulo de exportação não disponível. Verifique se `exportador.py` está no repositório.")
        else:
...
def main():
    # Show import errors and stop if critical modules missing
    if _import_errors:
        _err_msg = "\n".join(_import_errors)
        st.error("❌ Erro(s) ao importar módulos:\n" + _err_msg)
        st.info("Copie o erro acima e envie para o suporte.")
        if cal is None or Banco is None:
            st.stop()
